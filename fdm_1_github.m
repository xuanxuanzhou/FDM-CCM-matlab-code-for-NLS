...
%first-order, second-order in time and space, respectively
%CNFD—I
%参数与文章保持一致
%upload to github
%分割为两个区域计算，中间导数边界为一阶近似
%未内化，在边界不用方程
%nonlinear term: fraction
clc;clear;
tic
Tt=1;a=20;
...
for fM=1:5
    ...
    u=zeros(2*M-1,2);
    ue=zeros(2*M-1,1);non=ue;u0=ue;uxx=ue;x=ue;eu=ue;
    ...
    dl(M-1)=1/h;dm(M)=-2/h-lambda; dr(M)=1/h;
    fl=-fl/2;fr=-fr/2;fm=1i/tau*E-fm/2;
    fl(M-1)=-1/h;fm(M)=2/h+lambda; fr(M)=-1/h;
    ...
    for ip=4:4 %index_p
        ...
        for iper=1:1  %index_perturbation
            ...
            %                 end
            %                 mas(1,ip)=mas(1,ip)-h*abs(u(M,1))^2;
            %                 ene(1,ip)=ene(1,ip)-2*mu/(p+1)*h*abs(u(M,1))^(p+1)+lambda*(abs(u(M,1)))^2 ;
            %% computing
            for kk=1:N
                %kk
                non1=zeros(2*M-1,1);
                ...
                while ee>10^(-8)
                    %fixed point iteration: slowly
                    for k=1:2*M-1
                        if abs(u0(k))==abs(u(k,1))
                            non(k,1)=mu*abs(u(k,1))^(p-1)*(u0(k)+u(k,1))/2;
                            
                        else
                            non(k,1)=mu/(p+1)*(-abs(u0(k))^(p+1)+abs(u(k,1))^(p+1))/(-abs(u0(k))^2+abs(u(k,1))^2)*(u(k,1)+u0(k));
                        end
                    end
                    non(M,1)=0;
                ...
                %                     end
                %                     mas(kk+1,ip)=mas(kk+1,ip)-h*abs(u(M,1))^2;
                %                     ene(kk+1,ip)=ene(kk+1,ip)-2*mu/(p+1)*h*abs(u(M,1))^(p+1)+lambda*abs(u(M,1))^2 ;
            end%kk
            %% error analysis
            eu=u(:,1)-ue;
            for k=1:2*M-1
